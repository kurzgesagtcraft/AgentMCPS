...
class Tree:
    ...
    def get_state(self,active_window_handle:int|None,other_windows_handles:list[int],use_dom:bool=False)->TreeState:
        ...
        if active_window_handle:
            ...
            if use_dom:
                # Optimization: When using DOM, focus exclusively on the active window
                # to prevent timeouts caused by traversing multiple background applications.
                windows_handles=[active_window_handle]
            else:
    ...
    def get_windowwise_nodes(self,windows_handles:list[int],active_window_flag:bool,use_dom:bool=False) -> tuple[list[TreeElementNode],list[ScrollElementNode],list[TextElementNode]]:
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for handle, is_browser in task_inputs:
                # Prioritize active window with more depth
                depth = 12 if handle == windows_handles[0] and active_window_flag else 5
                if use_dom and is_browser:
                    depth = 15 # DOM needs more depth
                
                future = executor.submit(self.get_nodes, handle, is_browser, use_dom, depth)
                future_to_handle[future] = handle
    ...
    def tree_traversal(self, node: Control, window_bounding_box:Rect, window_name:str, is_browser:bool,
                    ...
                    is_dom:bool=False, is_dialog:bool=False, depth:int=0, max_depth:int=15,
                    element_cache_req:Optional[Any]=None, children_cache_req:Optional[Any]=None,
                    context:Optional[dict]=None):
        try:
            if context is None:
                context = {'node_count': 0, 'start_time': time(), 'max_nodes': 1200, 'timeout': 45.0}

            # Hard limits to prevent timeouts
            if depth > max_depth or context['node_count'] > context['max_nodes']:
                return
            
            if time() - context['start_time'] > context['timeout']:
                logger.warning(f"Tree traversal timeout reached for {window_name} at depth {depth}")
                return
            ...
            if is_visible:
                ...
                if is_enabled:
                    ...
                    if interactive_nodes is not None:
                        ...
                        # Optimization: Use pre-defined sets for faster lookup
                        BROWSER_INTERACTIVE_SKIP = set(['DataItemControl', 'ListItemControl'])
                        
                        if is_browser and control_type_name in BROWSER_INTERACTIVE_SKIP and not is_keyboard_focusable:
                            is_interactive = False
                            ...
                            is_interactive = True
                             ...
                             # Role check - Only fetch LegacyIAccessiblePattern if necessary
                             ...
                             if is_browser:
                                 ...
                                 try:
                                    ...
                                    if legacy_pattern:
                                        is_role_interactive = AccessibleRoleNames.get(legacy_pattern.Role, "Default") in INTERACTIVE_ROLES
                                        if legacy_pattern.DefaultAction and legacy_pattern.DefaultAction.title() in DEFAULT_ACTIONS:
                        ...
                        if is_interactive:
                            # Reuse legacy_pattern if already fetched in interactive check
                            try:
                                if 'legacy_pattern' not in locals() or legacy_pattern is None:
                                    legacy_pattern = node.GetLegacyIAccessiblePattern()
                                value = legacy_pattern.Value.strip() if legacy_pattern and legacy_pattern.Value is not None else ""
                            except Exception:
                                value = ""
            ...
            for child in (children if is_dom else children[::-1]):
                ...
                if is_browser and not is_dom and child.CachedAutomationId=="RootWebArea":
                    ...
                    self.dom=child
                    # enter DOM subtree
                    self.tree_traversal(child, window_bounding_box, window_name, is_browser, interactive_nodes, scrollable_nodes, dom_interactive_nodes, dom_informative_nodes, is_dom=True, is_dialog=is_dialog, depth=0, max_depth=max_depth, element_cache_req=element_cache_req, children_cache_req=children_cache_req, context=context)
                    
                    # Optimization: If we are only looking for DOM, we can stop searching other siblings once found
                    if not interactive_nodes and dom_interactive_nodes:
                        return
                ...
                # Check if the child is a dialog
                    ...
                    self.tree_traversal(child, window_bounding_box, window_name, is_browser, interactive_nodes, scrollable_nodes, dom_interactive_nodes, dom_informative_nodes, is_dom=is_dom, is_dialog=True, depth=depth+1, max_depth=max_depth, element_cache_req=element_cache_req, children_cache_req=children_cache_req, context=context)
                else:
                    # normal non-dialog children
                    self.tree_traversal(child, window_bounding_box, window_name, is_browser, interactive_nodes, scrollable_nodes, dom_interactive_nodes, dom_informative_nodes, is_dom=is_dom, is_dialog=is_dialog, depth=depth+1, max_depth=max_depth, element_cache_req=element_cache_req, children_cache_req=children_cache_req, context=context)
        except Exception as e:
    ...
    def get_nodes(self, handle: int, is_browser:bool=False, use_dom:bool=False, max_depth:int=5) -> tuple[list[TreeElementNode],list[ScrollElementNode],list[TextElementNode]]:
        try:
            ...
            # Fast Path for Browser DOM
            if use_dom and is_browser:
                try:
                    # Try to find RootWebArea directly to skip browser UI traversal
                    from windows_mcp.uia import PropertyId, ControlType
                    
                    # Optimization: Search for DocumentControl which is usually the RootWebArea
                    # Optimization: Use a shallower scope first or specific search
                    # RootWebArea is usually not very deep in the browser window
                    root_web_area = node.FindFirst(TreeScope.TreeScope_Children | TreeScope.TreeScope_Descendants,
                                                 CacheRequestFactory.create_tree_traversal_cache(),
                                                 PropertyId.AutomationIdProperty, "RootWebArea")
                    
                    if not root_web_area:
                        # Fallback to searching by control type if AutomationId fails
                        root_web_area = node.FindFirst(TreeScope.TreeScope_Children | TreeScope.TreeScope_Descendants,
                                                     CacheRequestFactory.create_tree_traversal_cache(),
                                                     PropertyId.ControlTypeProperty, ControlType.DocumentControl)

                    if root_web_area:
                        root_web_area._is_cached = True
                        bounding_box = root_web_area.CachedBoundingRectangle
                        self.dom_bounding_box = BoundingBox(left=bounding_box.left, top=bounding_box.top,
                                                          right=bounding_box.right, bottom=bounding_box.bottom,
                                                          width=bounding_box.width(), height=bounding_box.height())
                        self.dom = root_web_area
                        self.tree_traversal(root_web_area, window_bounding_box, window_name, is_browser,
                                          None, scrollable_nodes, dom_interactive_nodes, dom_informative_nodes,
                                          is_dom=True, is_dialog=False, depth=0, max_depth=10,
                                          element_cache_req=element_cache_req, children_cache_req=children_cache_req)
                        return (dom_interactive_nodes, scrollable_nodes, dom_informative_nodes)
                except Exception as e:
                    logger.debug(f"Fast DOM path failed: {e}")

            self.tree_traversal(node, window_bounding_box, window_name, is_browser, interactive_nodes, scrollable_nodes, dom_interactive_nodes, dom_informative_nodes, is_dom=False, is_dialog=False, depth=0, max_depth=max_depth, element_cache_req=element_cache_req, children_cache_req=children_cache_req, context=context)
            ...
            if use_dom:
                if is_browser and dom_interactive_nodes:
                    return (dom_interactive_nodes,scrollable_nodes,dom_informative_nodes)
                else:
                    # Fallback: If use_dom is requested but no DOM nodes found (or not a browser),
                    # return standard interactive nodes so the user isn't left with an empty result.
                    return (interactive_nodes,scrollable_nodes,dom_informative_nodes)
            else:
