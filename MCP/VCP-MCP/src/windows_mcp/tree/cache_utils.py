...
class CachedControlHelper:
    ...
    def get_cached_children(node: Control, cache_request: Optional[CacheRequest] = None) -> list[Control]:
        ...
        try:
            # If node is already cached AND it was cached with Children scope, use it
            if getattr(node, '_is_cached', False):
                try:
                    ...
                    # If GetCachedChildren fails, it might not have been cached with Children scope
                    pass

            # Build updated cache that includes children
            # IMPORTANT: Ensure TreeScope includes Children for this call
            cache_request.TreeScope = TreeScope.TreeScope_Element | TreeScope.TreeScope_Children
