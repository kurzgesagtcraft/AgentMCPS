...
def state_tool(use_vision:bool=False,use_dom:bool=False, focus_window:Optional[str]=None, only_active_window:bool=False, ctx: Context = None):
    """
    Captures desktop state.
    - use_vision: Include screenshot.
    - use_dom: Get web content instead of browser UI.
    - focus_window: Optional name of window to focus on (improves speed).
    - only_active_window: Only scan the foreground window (fastest).
    """
    if focus_window:
    ...
    desktop_state=desktop.get_state(use_vision=use_vision,use_dom=use_dom,as_bytes=True,scale=scale,only_active_window=only_active_window)
