...
@mcp.tool(
    name='Snapshot',
    description='Captures complete desktop state including: system language, focused/opened windows, interactive elements (buttons, text fields, links, menus with coordinates), and scrollable areas. Set use_vision=True to include screenshot. Set use_dom=True for browser content to get web page elements instead of browser UI. Set only_active_window=True for maximum speed by only scanning the foreground window. Always call this first to understand the current desktop state before taking actions.',
    annotations=ToolAnnotations(
        title="Snapshot",
        readOnlyHint=True,
...
def state_tool(
    ...
    only_active_window: bool | str = False,
    ctx: Context = None,
):
    try:
        ...
        only_active_window = only_active_window is True or (
            isinstance(only_active_window, str) and only_active_window.lower() == "true"
        )

        if focus_window:
            handle = desktop.get_window_by_name(focus_window)
            if handle:
                desktop.bring_window_to_top(handle)
                # Small sleep to allow UI to settle
                time.sleep(0.5)
        ...
        desktop_state = desktop.get_state(
            ...
            as_bytes=False,
            scale=scale,
            only_active_window=only_active_window,
        )
...
def click_tool(
    ...
    include_visualization: bool | str = False,
    ctx: Context = None,
):
    include_visualization = include_visualization is True or (
        isinstance(include_visualization, str) and include_visualization.lower() == "true"
    )
    if element_name:
    ...
    message = f"{num_clicks.get(clicks)} {button} clicked at ({x},{y}){source}."

    if include_visualization:
        # Get visualization screenshot
        viz_img = desktop.get_click_visualization(loc)

        # Scale if needed (consistent with Snapshot tool)
        scale_width = (
            MAX_IMAGE_WIDTH / screen_size.width if screen_size.width > MAX_IMAGE_WIDTH else 1.0
        )
        scale_height = (
        ...
        if scale != 1.0:
            ...
            viz_img = viz_img.resize(
                (int(viz_img.width * scale), int(viz_img.height * scale)),
                PILImage.Resampling.LANCZOS,
            )

        # Convert to bytes for MCP Image type
        buffered = io.BytesIO()
        viz_img.save(buffered, format="PNG")
        img_bytes = buffered.getvalue()
        buffered.close()

        return [message, Image(data=img_bytes, format="png")]
