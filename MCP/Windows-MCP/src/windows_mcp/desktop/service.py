...
class Desktop:
    ...
    def get_state(
        ...
        if only_active_window:
            # Optimization: skip full window enumeration
            active_window = self.get_active_window(windows=[])
            windows = []
            windows_handles = {active_window.handle} if active_window else set()
            controls_handles = windows_handles.copy()
        else:
    ...
    def get_window_by_name(self, name: str) -> int | None:
        """Get window handle by name using fuzzy matching."""
        windows, _ = self.get_windows()
        windows_dict = {window.name: window.handle for window in windows}
        matched = process.extractOne(name, list(windows_dict.keys()), score_cutoff=60)
        if matched:
            return windows_dict[matched[0]]
        return None
    ...
    def get_click_visualization(self, loc: tuple[int, int]) -> Image.Image:
        """Create a visualization of a click at the specified location."""
        screenshot = self.get_screenshot()
    ...
    def type(
        ...
        # Use clipboard for CJK characters to ensure proper input
        if self._contains_cjk(text):
            try:
                ...
                logger.warning(f"Clipboard input failed for CJK text: {e}, falling back to SendKeys")
