...
class MCPOPlugin:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        config = {
            'MCPO_PORT': int(os.getenv('MCPO_PORT', '9999')),
            ...
            'MCPO_TIMEOUT': int(os.getenv('MCPO_TIMEOUT', '120000')) // 1000  # 转换为秒，默认120秒
        }
    ...
    def _find_mcpo_executable(self) -> str:
        """查找 mcpo 可执行文件的完整路径"""
        # 可能的 mcpo 路径列表
        possible_paths = [
            # miniconda3 Scripts 目录
            r'D:\miniconda3\Scripts\mcpo.exe',
            # 用户 Scripts 目录
            os.path.expanduser(r'~\AppData\Roaming\Python\Python312\Scripts\mcpo.exe'),
            # 使用 sys.executable 推断
            os.path.join(os.path.dirname(sys.executable), 'Scripts', 'mcpo.exe'),
            os.path.join(os.path.dirname(sys.executable), 'mcpo.exe'),
        ]
        
        for path in possible_paths:
            if os.path.isfile(path):
                self.logger.info(f"Found mcpo executable at: {path}")
                return path
        
        # 如果都找不到，尝试使用 shutil.which
        import shutil
        mcpo_path = shutil.which('mcpo')
        if mcpo_path:
            self.logger.info(f"Found mcpo in PATH: {mcpo_path}")
            return mcpo_path
        
        # 最后回退到简单的 'mcpo' 命令，让系统在 PATH 中查找
        self.logger.warning("Could not find mcpo executable, falling back to 'mcpo' command")
        return 'mcpo'
    ...
    def _start_mcpo_server(self) -> bool:
        ...
        try:
            ...
            # 构建启动命令 - 使用完整的 mcpo 路径
            mcpo_cmd = self._find_mcpo_executable()
            cmd = [
                mcpo_cmd,
                '--config', config_path,
            ...
            # 注意: MCPO不支持--hot-reload参数，已移除此功能
    ...
    def _make_request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        # 使用配置的超时时间，默认120秒
        timeout = self.config.get('MCPO_TIMEOUT', 120)
