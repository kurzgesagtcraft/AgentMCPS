...
const apiConfig = require('./modules/apiConfig');
...
class KnowledgeBaseManager {
    constructor(config = {}) {
        this.config = {
            ...
            apiUrl: apiConfig.embedding.apiUrl,
            model: apiConfig.embedding.model,
            // ⚠️ 务必确认环境变量 VECTORDB_DIMENSION 与模型一致 (3-small通常为1536)
            dimension: apiConfig.embedding.dimension,
    ...
    // === 诊断与统计 API (适配旧版 VectorDBStorage 接口) ===

    getStats() {
        if (!this.db) return { diaryCount: 0, chunkCount: 0, fileCount: 0, dbSize: 0 };
        try {
            const diaryCount = this.db.prepare("SELECT COUNT(DISTINCT diary_name) as count FROM files").get().count;
            ...
            const dbPath = path.join(this.config.storePath, 'knowledge_base.sqlite');
            const dbSize = fsSync.existsSync(dbPath) ? fsSync.statSync(dbPath).size : 0;

            return { diaryCount, chunkCount, fileCount, dbSize };
        } catch (e) {
            console.error('[KnowledgeBase] Failed to get stats:', e);
            return { diaryCount: 0, chunkCount: 0, fileCount: 0, dbSize: 0 };
        }
    }
    ...
    getAllDiaryNames() {
        if (!this.db) return [];
        try {
            const rows = this.db.prepare("SELECT DISTINCT diary_name FROM files").all();
            return rows.map(r => r.diary_name);
        } catch (e) {
            return [];
        }
    }
    ...
    getFileHashes(diaryName) {
        if (!this.db) return {};
        try {
            const rows = this.db.prepare("SELECT path, checksum FROM files WHERE diary_name = ?").all(diaryName);
            const hashes = {};
            rows.forEach(r => {
                hashes[path.basename(r.path)] = r.checksum;
            });
            return hashes;
        } catch (e) {
            return {};
    ...
    getChunkMap(diaryName) {
        ...
        try {
            ...
            const chunkMap = {};
            rows.forEach(r => {
                chunkMap[r.id] = { text: r.content, sourceFile: r.path };
            });
            return chunkMap;
        } catch (e) {
            return {};
    ...
    async shutdown() {
        ...
        // 3. 确保所有待保存的索引都被写入磁盘并清理定时器
        for (const [name, timer] of this.saveTimers) {
            clearTimeout(timer);
            try {
                this._saveIndexToDisk(name);
            } catch (e) {
                console.error(`[KnowledgeBase] Final save failed for ${name}:`, e.message);
            }
        }
